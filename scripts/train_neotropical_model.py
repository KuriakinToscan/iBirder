...
#  Script de Treinamento & Quantização: Modelo iBirder Neotropical (EfficientNetV2 TFLite)
...
def gerar_script_treinamento_colab():
    """
    Gera o notebook / script em Python pré-configurado para execução em ambiente GPU 
    (Google Colab / Kaggle / Servidor Local GPU) para treinamento da rede EfficientNetV2 Neotropical.
    """
    colab_script = """# iBirder Neotropical Model Training Pipeline
...
"""
    script_path = BASE_DIR / "scripts" / "colab_train_pipeline.py"
    with open(script_path, "w", encoding="utf-8") as f:
        f.write(colab_script)
    logging.info(f"Pipeline de treinamento Colab/GPU gerado em: {script_path}")

if __name__ == "__main__":
    logging.info("=== Preparando Pipeline de Treinamento da IA Neotropical ===")
    gerar_script_treinamento_colab()
    logging.info("Sucesso! Pipeline de ML Ops pronto para execução em ambiente GPU.")
